...
class HAL_file():
    ...
    def parse_files(self, insert_file):  # this function will return the file in a format that can be asserted.
...
for num in list_to_send:
    ...
    doc = current_file.parse_files(num)  # sets the doc variable to contain the parsed file content from the pre-uploaded file
    #using the class parse_file method
    post_xml.main(num)  # calls the function to post the file to the endpoint

    compare_data = (current_file.get('https://firstqtooltest.s3.eu-west-2.amazonaws.com/' + num).text) # sets the content of the uploaded file
    uploaded_file_url = (current_file.get('https://firstqtooltest.s3.eu-west-2.amazonaws.com/file2.xml').url) # sets the url of the uploaded file
